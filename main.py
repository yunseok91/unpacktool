...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.server_group.addButton(self.radioButton_3)
        ...
        # 새로 추가된 부분: 입력란 텍스트 변경 이벤트 연결
        self.lineEdit_id.textChanged.connect(self.toggle_input_fields)
        self.lineEdit_password.textChanged.connect(self.toggle_input_fields)
        self.lineEdit_cookie.textChanged.connect(self.toggle_input_fields)
    ...
    def toggle_input_fields(self):
        ...
        if self.radioButton_2.isChecked():
            ...
            self.lineEdit_cookie.hide()
            #새로 추가된 부분: ID와 PW 체크하여 Run 버튼 활성화/비활성화
            id_text = self.lineEdit_id.text().strip()
            pw_text = self.lineEdit_password.text().strip()
            self.runButton.setEnabled(bool(id_text and pw_text))

        elif (self.radioButton_3.isChecked()): 
            self.lineEdit_id.hide()
            self.lineEdit_password.hide()
            self.lineEdit_cookie.show()
            # 새로 추가된 부분: 쿠키 체크하여 Run 버튼 활성화/비활성화
            cookie_text = self.lineEdit_cookie.text().strip()
            self.runButton.setEnabled(bool(cookie_text))
    ...
    def run_function(self):
        ...
        if self.radioButton_1.isChecked():
            ...
            cookie = None  # QA 모드에서는 쿠키 사용 안 함
            self.logger.log(f" ID: {username} \nPWD: {password}", "info")
            ...
            # 쿠키 값이 비어있는지 확인
            if not cookie:
                self.logger.log(" WMC 모드에서는 쿠키 값이 필요합니다 ")
                return
            # 쿠키에 login-token= 프리픽스가 없으면 자동 추가
            if not "login-token=" in cookie and not "document.cookie=" in cookie:
                cookie = f"login-token={cookie}; path=/"
                self.logger.log(f"쿠키 형식 자동 변환: {cookie}", "info")
        else:
            server_type = "WWW"
            username = None
            password = None
...
if __name__ == "__main__":
    try:
        ...
        print(f"main함수 exception: {str(e)}")
